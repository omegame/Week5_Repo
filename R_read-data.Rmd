...
```{r}
library(here)
fev_file <- read_file(here("Week5_Repo.Rproj"))
```
...
```{r}
# Add your code here!
fev_data <- read_csv("data/fev.csv", show_col_types = FALSE) %>%
  filter(sex == 1) %>%
  select(fev, age)
```
...
```{r}
# Add your code here!
psa_data <- read.table(here("data/psa.txt"), header = TRUE)
summary(psa_data)
```
...
```{r}
fev_data <- read_csv("data/fev.csv", col_types = cols(fev = col_double(), height = col_double()), na = "NA")
head(fev_data)
```
